...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  // null = "All People" — stores the display NAME (not UID)
  String? selectedPersonName;
  String? selectedPersonLabel;

  // uid → display name map, kept in sync from the users stream
  ...
  /// Returns sorted unique list of display NAMES found in docs,
  /// resolving paidBy UIDs → names via [uidToName].
  List<String> _uniquePersonNames(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
    Map<String, String> uidToName,
  ) {
    final set = <String>{};
    for (final doc in docs) {
      final uid = doc.data()['paidBy'] as String?;
      if (uid == null || uid.trim().isEmpty) continue;
      final name = uidToName[uid.trim()] ?? uid.trim();
      set.add(name);
    }
  ...
  // ── PERSON BOTTOM SHEET ──
  void _openPersonPicker(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
    Map<String, String> uidToName,
  ) {
    // persons is a list of display NAMES
    final persons = _uniquePersonNames(docs, uidToName);
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (persons.isNotEmpty) ...[
                        ...
                        // Each item p is a display NAME
                        ...persons.map((p) => _categoryTile(
                              icon: '👤',
                              label: p,
                              id: p,
                              currentId: selectedPersonName,
  ...
  Future<void> _downloadPdf(
    ...
    setState(() => _isGeneratingPdf = true);
    try {
      var filteredDocs = selectedCategoryId != null
          ? docs
              .where((d) => d.data()['category'] == selectedCategoryId)
              .toList()
          : docs;
      await PdfService.downloadReport(
        ...
        partnerName: 'Swastik Hangers',
        customCategoryNames: customCategoryNames,
        // personFilter is a NAME (e.g. "Dhanaji Arage") or null for all
        personFilter: selectedPersonName,
        uidToName: uidToName,
        context: context,
      );
    } catch (e) {
  ...
  @override
  Widget build(BuildContext context) {
    // Outermost stream: resolve all users (uid → name)
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('users').snapshots(),
      builder: (context, userSnapshot) {
        // Build uid→name lookup from the users collection
        final Map<String, String> uidToName = {};
        if (userSnapshot.hasData) {
          for (final doc in userSnapshot.data!.docs) {
            final name = doc.data()['name'] as String?;
            if (name != null && name.trim().isNotEmpty) {
              uidToName[doc.id] = name.trim();
            }
          }
        }
        // Keep in sync so PDF calls can use it
            ...
            return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (selectedCategoryId != null) {
                  summaryDocs = summaryDocs
                      .where(
                          (d) => d.data()['category'] == selectedCategoryId)
                      .toList();
                }

                // Filter by person — selectedPersonName is a display NAME;
                // find all UIDs that resolve to that name and filter by them.
                if (selectedPersonName != null) {
                  final matchingUids = uidToName.entries
                      .where((e) => e.value == selectedPersonName)
                      .map((e) => e.key)
                      .toSet();

                  summaryDocs = summaryDocs
                      .where((d) =>
                          matchingUids.contains(d.data()['paidBy']))
                      .toList();
                }
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () =>
                                _openPersonPicker(docs, uidToName),
                            child: AnimatedContainer(
                              duration: const Duration(milliseconds: 180),
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 16, vertical: 10),
                              decoration: BoxDecoration(
                                color: selectedPersonName != null
                                    ? const Color(0xFF6366F1)
                                    : const Color(0xFF1F2937),
                                borderRadius: BorderRadius.circular(18),
                              ),
                      ...
                      // ── TOP SPENDING ──
                      _topSpendingSection(
                          summaryEntries, customCategoryNames),
                      const SizedBox(height: 24),
